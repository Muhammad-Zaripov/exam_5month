...
class Coffeebottomsheet extends StatefulWidget {
  final void Function(List<FoodItem>) onSelected;
  const Coffeebottomsheet({super.key, required this.onSelected});

  @override
  State<Coffeebottomsheet> createState() => _CoffeebottomsheetState();
}

class _CoffeebottomsheetState extends State<Coffeebottomsheet> {
  final List<FoodItem> selected = [
    FoodItem(name: "Espresso", quantity: 0),
    FoodItem(name: "Cappuccino", quantity: 0),
    FoodItem(name: "Latte", quantity: 0),
    FoodItem(name: "Americano", quantity: 0),
  ];
  ...
  Future<void> submitToFirebase(List<FoodItem> items) async {
    ...
    for (var item in filtered) {
      await _dbRef.child("orders/coffees/${item.name}").set({
        'name': item.name,
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(hintText: "Yangi qahva nomi"),
                  ),
                ),
                IconButton(
